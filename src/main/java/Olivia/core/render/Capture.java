...
public class Capture {
    ...
    public Capture(VisualisationManager visualisationManager) {
    ...
    public void toggleCaptureVideo() {
        if (!captureVideo) {
            captureVideo = true;
            visualisationManager.getGUI().logIntoConsole("Recording...");
        } else if (captureVideo) {
            //try {
                videoWriters[0].close();
                videoWriters[1].close();
                captureVideo = false;
                visualisationManager.getGUI().logIntoConsole("Video recorded");
                //rotateVideo();
            //}
    ...
    public void screenshot() {
        ...
        try {
            ...
            visualisationManager.getRenderScreen().getGl2().glReadPixels(0, 0, width, height, GL2.GL_RGBA, GL2.GL_UNSIGNED_BYTE, buf);
    ...
    private void createVideo(int side) {
        ...
        //int fourcc = CV_FOURCC((byte) 'H', (byte) '2', (byte) '6', (byte) '4');
        updateResolution();
    ...
    public void recordVideo() {
        if (!videoWriters[0].isOpened()) {
            ...
            visualisationManager.getRenderScreen().getGl2().glReadPixels(0, 0, width, height, GL2.GL_RGBA, GL2.GL_UNSIGNED_BYTE, pixelBuffs[0]);
    ...
    public void recordVideo3D(int side) {
        if (!videoWriters[side].isOpened()) {
            ...
            if (side == LEFT_SIDE) {
                visualisationManager.getRenderScreen().getGl2().glReadBuffer(GL2GL3.GL_BACK_LEFT);
            } else {
                visualisationManager.getRenderScreen().getGl2().glReadBuffer(GL2GL3.GL_BACK_RIGHT);
            }
            visualisationManager.getRenderScreen().getGl2().glReadPixels(0, 0, width, height, GL2.GL_RGBA, GL2.GL_UNSIGNED_BYTE, pixelBuffs[side]);
